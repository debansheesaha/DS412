...
shapiro.test(x)#test for normality Shapiro - Wilk normality test data: x
...
###For the mean, we have a formula for the standard error, namely n, where n is the population standard deviation and n is the sample size. This can be estimated for any sample by Sh = s n. This estimate should be acceptable for moderate to large sample sizes ( n at least 30 or more):
```{r}
set.seed(1) #for reproducibility
u0 <- mean(x) #mean of original sample
Sh <- sd(x)/sqrt(n)
thetas <- NULL
tstar <- NULL
for (i in 1:1000) { #bootstrap resampling
 xx <- sample(x, n, replace = TRUE) #new resample
 u <- mean(xx) #estimate
 thetas[i] <-u #save
 tstar[i] <- ((u-u0)/sd(xx)/sqrt(n)) #pivotal quantity
 }
 c(u0, mean(thetas)) #compare sample mean to mean of bootstrap sampling distribution
 c(Sh, sd(thetas)) #compare standard error from sample to standard error estimate from bootstrap distribution
 summary(tstar)
 ...
 u0 + quantile(tstar, probs = c(0.025,0.975)) * Sh #bootstrap percentile t confidence interval
 u0 + qt(c(0.025,0.975), n - 1) * Sh #student t confidence interval for comparison
```
...
{sqrt(var(x)/length(x))},
 nboott = 1000, perc = c(0.025,0.975)) #bootstrap percentile t 95% C.I.
```
...
set.seed(1) #for reproducibility
require('boot')
fboot<- function(x, i) mean(x[i]) #compute estimate given data x and index set i
...
abc.ci(x, fabc, conf=0.95) #ABC method C.I.
```
###The 95% confidence interval estimates for the BCa and ABC methods are (6.75, 13.30)and (6.863, 13.157), comparable to those found before. It should be noted that the ABC method is both very fast computationally and very accurate, typically giving intervals very close to those obtained from the BCa method.

###(d)Find a 95% confidence interval on the mean using the percentile-t method
```{r}
set.seed(1) #reproducibility
boott(x, theta = mean, nbootsd = 100, nboott = 1000,
perc = c(0.025,0.975)) #bootstrap percentile t 95% C.I. with nested bootstrap
```
...
```{r}
 ...
 n <- length(x) #sample size
 n
 mean(x) + qt(0.950, n - 1) * sd(x) * c( - 1, + 1)/sqrt(n) #student t based 90% CI
 u0 <- mean(x) #mean of original sample
 Sh <- sd(x)/sqrt(n)
 thetas <- NULL
 tstar <- NULL
 ...
 c(Sh, sd(thetas)) #compare standard error from sample to standard error estimate from bootstrap distribution
 summary(tstar)
 quantile(tstar, probs = c(0.05,0.95)) #quantiles from bootstrap percentile t
 qt(c(0.050,0.950), n - 1) #quantiles from student t distribution
 u0 + quantile(tstar, probs = c(0.05,0.95)) * Sh #bootstrap percentile t confidence interval
 u0 + qt(c(0.05,0.95), n - 1) * Sh #student t confidence interval for comparison
  #using nested bootstrap
 set.seed(1) #reproducibility
 require('bootstrap')
 boott(x, theta = mean, sdfun = function(x,nbootsd,theta)
 {sqrt(var(x)/length(x))},
 nboott = 1000, perc = c(0.05,0.95)) #bootstrap percentile t 90% C.I.
```

###(b) Compare the alfatoxin level found with the industry average value of 5.7 ppm: Is the upper confidence limit less than 5.7 ppb, or is it equal or above? What does this imply about a hypothesis test of H0 : µ ??? 5.7 ppb versus H1 : µ < 5.7 ppb at the ?? = 0.05 significance level?
```{r}
 set.seed(1)
 x <-c(4.94, 5.06, 4.53, 5.07, 4.99, 5.16, 4.38, 4.43, 4.93, 4.72, 4.92, 4.96)
 summary(x)
 ...
 n <- length(x) #sample size
 n
 mean(x) + qt(0.975, n - 1) * sd(x) * c( - 1, + 1)/sqrt(n) #student t based 90% CI
 ...
 quantile(tstar, probs = c(0.025,0.975)) #quantiles from bootstrap percentile t
 qt(c(0.025,0.975), n - 1) #quantiles from student t distribution
 u0 + quantile(tstar, probs = c(0.025,0.975)) * Sh #bootstrap percentile t confi dence interval
 u0 + qt(c(0.025,0.975), n - 1) * Sh #student t confidence interval for comparison
...
#nt is total relevant
#nc is total irrelevant
#xt is relevant retrived
#xc is irrelevant retrived
#numresamp is number of resamples 
nt <- 150
nc <- 7328
xt <- 123
xc <- 625
...
```{r}
#Efron percentile based on 90% CI
quantile(simdiffs, probs = c(0.05,0.95))

#Efron percentile based on 95% CI
